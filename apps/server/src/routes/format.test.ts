...
describe('Buchformat wechseln', () => {
  it('setzt das Profil und liefert die neuen Maße', async () => {
    ...
    const res = await app.inject({
      method: 'PATCH',
      url: '/api/format',
      payload: { printProfileId: 'saal-42x28' },
    });

    expect(res.statusCode).toBe(200);
    expect(project.settings.printProfileId).toBe('saal-42x28');
  ...
  it('ordnet dabei nichts neu', async () => {
    ...
    await app.inject({
      method: 'PATCH',
      url: '/api/format',
      payload: { printProfileId: 'saal-15x15' },
    });
  ...
  it('lehnt eine unbekannte Kennung mit einem Satz ab', async () => {
    ...
    const res = await app.inject({
      method: 'PATCH',
      url: '/api/format',
      payload: { printProfileId: 'saal-99x99' },
    });

    expect(res.statusCode).toBe(400);
    expect((res.json() as { error: string }).error).toContain('saal-99x99');
    // Und ändert nichts: Ein abgelehnter Wechsel darf kein halbes Format setzen.
    expect(project.settings.printProfileId).toBe('saal-28x28');
  });
});
